...
boru-driven command-line client **and** interactive REPL for the Discard
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/discard-cli)
make build
...
DISCARD_BASE=https://api.example.com ./discard-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/discard-cli
   ```
...
Run with no arguments to open a REPL (prompt `discard>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `ai_chat`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/discard-cli`. |
...
(`./discard-cli <expr>`) and interactively (the REPL), and expressions compose the

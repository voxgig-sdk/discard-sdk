// Command Discard-cli is a CLI + REPL for the Discard SDK,
...
const prompt = "discard"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "ai_chat test testing"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DISCARD_APIKEY (api key), DISCARD_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DISCARD_APIKEY carries the API key and
	// DISCARD_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DISCARD_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DISCARD_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDiscardSDK(opts)

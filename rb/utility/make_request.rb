...
module DiscardUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = DiscardResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DiscardResponse.new(fetched)
    else

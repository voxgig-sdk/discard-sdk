...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = DiscardOmni.make_runner(TEST_JSON_FILE, DiscardSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : DiscardOmni.errify(err)
    end

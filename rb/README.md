...
The Ruby SDK for the Discard API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
# Create
created, _ = client.AiChat(nil).create({ "name" => "Example" }, nil)
...
result, err = client.Discard(nil).load(
  { "id" => "test01" }, nil
)

...
DISCARD API is a fast and reliable REST API platform built with Go, designed to facilitate seamless interactions for developers.

Learn more about DISCARD API at [discardapi.dpdns.org](https://discardapi.dpdns.org).
...
This SDK exposes the API as a small set of **semantic entities** — AiChat, Test and Testing — that you
...
const aichat = await client.AiChat().create({
  message: 'example',
})
```
...
```go
client := sdk.Test()
result, err := client.Test(nil).Load(

...
from discard_sdk.discard_types import (
    Testing,
    TestingLoadMatch,
    TestingCreateData,
)
...
class TestingEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TestingEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Testing:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Testing().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: TestingCreateData, ctrl=None) -> Testing:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,

...
class DiscardTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DiscardTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DiscardTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DiscardTestRunner.getenv("DISCARD_TEST_LIVE")
        override = DiscardTestRunner.getenv("DISCARD_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DiscardTestRunner.getenv(key)
        ...
        explain = DiscardTestRunner.getenv("DISCARD_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["DISCARD_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if DiscardTestRunner._test_control is not None:
            return DiscardTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DiscardTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DiscardTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DiscardTestRunner.load_test_control()
...
def load_env_local():
    DiscardTestRunner.load_env_local()


def env_override(m):
    return DiscardTestRunner.env_override(m)


def entity_data(v):
    return DiscardTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DiscardTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DiscardTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DiscardTestRunner.load_test_control()


def live_client_options():
    return DiscardTestRunner.live_client_options()


def live_delay_ms():
    return DiscardTestRunner.live_delay_ms()

...
class TestTestingEntity:

    def test_should_create_instance(self):
        testsdk = DiscardSDK.test(None, None)
        ent = testsdk.Testing(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _testing_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "testing." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DISCARD_TEST_TESTING_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        testing_ref01_ent = client.Testing(None)
        testing_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.testing"), "testing_ref01"))

        testing_ref01_data = helpers.to_map(runner.entity_data(testing_ref01_ent.create(testing_ref01_data, None)))
...
def _testing_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/testing/TestingTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("DISCARD_TEST_TESTING_ENTID"))

...
// Discard SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = DiscardOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(DiscardOmni::structfix($now, Struct::undef()))
                        !== json_encode(DiscardOmni::structfix($val, Struct::undef()))) {

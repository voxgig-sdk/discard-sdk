...
class DiscardNetsimFeature extends DiscardBaseFeature
{
    ...
    private function _simulate(DiscardContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(DiscardContext $ctx, array $applied): void
    {
        $client = $this->client;

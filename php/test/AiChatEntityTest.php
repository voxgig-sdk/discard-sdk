...
class AiChatEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = DiscardSDK::test(null, null);
        $ent = $testsdk->AiChat(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = ai_chat_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "ai_chat." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set DISCARD_TEST_AI_CHAT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $ai_chat_ref01_ent = $client->AiChat(null);
...
function ai_chat_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/ai_chat/AiChatTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["ai_chat01", "ai_chat02", "ai_chat03"] as $k) {
    ...
    $entid_env_raw = getenv("DISCARD_TEST_AI_CHAT_ENTID");

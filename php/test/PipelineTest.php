...
{
    private static function utility(): DiscardUtility
    {
        return new DiscardUtility();
    }
    ...
    private static function ctx(array $over = []): DiscardContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new DiscardContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new DiscardOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof DiscardError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new DiscardOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = DiscardMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new DiscardOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = DiscardMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new DiscardOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = DiscardMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DiscardSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = DiscardMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new DiscardResponse([]),
            'result' => new DiscardResult([]),
        ]);
        [, $err] = DiscardMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new DiscardSpec([]),
            'response' => null,
            'result' => new DiscardResult([]),
        ]);
        [, $err] = DiscardMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new DiscardSpec(['step' => 's']),
            'response' => new DiscardResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new DiscardResult([]),
        ]);
        [, $err] = DiscardMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new DiscardSpec(['step' => 's']),
            'response' => new DiscardResponse(self::resp(200, ['v' => 1])),
            'result' => new DiscardResult([]),
        ]);
        [, $err] = DiscardMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new DiscardSpec(['step' => 's']),
            'response' => new DiscardResponse(self::resp(200, ['v' => 2])),
            'result' => new DiscardResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        DiscardMakeResponse::call($ctx);
    ...
    {
        $preset = new DiscardResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new DiscardSpec([]),
            'response' => new DiscardResponse([]),
            'result' => new DiscardResult([]),
        ...
        [$got, $err] = DiscardMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new DiscardResult([])]);
        [, $err] = DiscardMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new DiscardSpec([]), 'result' => null]);
        [, $err] = DiscardMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DiscardSpec(['step' => 's']),
            'result' => new DiscardResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new DiscardOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DiscardMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DiscardSpec(['step' => 's']),
            'result' => new DiscardResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new DiscardOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DiscardMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DiscardResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new DiscardSpec([]),
            'result' => new DiscardResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = DiscardMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DiscardMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new DiscardError('boom', 'boom');
        $utility->fetcher = function (DiscardContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DiscardSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = DiscardMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (DiscardContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = DiscardMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(DiscardResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (DiscardContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DiscardSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = DiscardMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new DiscardResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new DiscardSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = DiscardMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = DiscardMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = DiscardMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new DiscardResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, DiscardDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new DiscardResult(['ok' => false])]);
        $this->expectException(DiscardError::class);
        DiscardDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new DiscardResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', DiscardMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        DiscardMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new DiscardBaseFeature();
        $b = new DiscardBaseFeature();
        DiscardFeatureAdd::call($ctx, $a);
        DiscardFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): DiscardBaseFeature
    {
        $f = new DiscardBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        DiscardFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): DiscardContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new DiscardSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DiscardPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new DiscardResponse(['headers' => null]),
            'result' => new DiscardResult([]),
        ]);
        DiscardResultHeaders::call($ctx);

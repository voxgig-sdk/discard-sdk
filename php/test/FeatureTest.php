...
class FtCtrl extends DiscardControl
{
...
class FtClient
{
    ...
    private DiscardUtility $utility;

    public function __construct(DiscardUtility $utility, array $options)
    {
    ...
    public function get_utility(): DiscardUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DiscardContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DiscardContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DiscardConfig::make_feature($name);
    ...
    private static function buildUrl(DiscardSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DiscardError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DiscardResponse($fetched);
        }
    ...
    private function fail(DiscardContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DiscardContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

...
type Args struct {
	Entity string         `json:"entity" jsonschema:"ai_chat | test | testing"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.DiscardSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "discard_list",
		Description: "List records from Discard. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "discard_load",
		Description: "Load a single record from Discard. " +
...
func runOp(client *sdk.DiscardSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.DiscardSDK, name string) (sdk.DiscardEntity, error) {
	switch strings.ToLower(name) {
	case "ai_chat":
		return client.AiChat(nil), nil
	case "test":
		return client.Test(nil), nil
	case "testing":
		return client.Testing(nil), nil

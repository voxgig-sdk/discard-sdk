...
[MCP](https://modelcontextprotocol.io) server exposing the Discard SDK as
two agent tools — `discard_list` and `discard_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/discard-mcp)
make build

# 2. Provide credentials via the environment
export DISCARD_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user discard \
  -- /absolute/path/to/discard-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./discard-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// discard_list: first page of records
{ "entity": "ai_chat" }
{ "entity": "ai_chat", "query": { } }

// discard_load: one record by id
{ "entity": "test", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/discard-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user discard \
     -- "$PWD"/dist/*/discard-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `discard_list` and `discard_load` tools now appear
   in new sessions. Ask the agent to *"list ai_chat using discard"*
   and it calls `discard_list` with `{"entity":"ai_chat"}`.
...
```sh
export DISCARD_APIKEY=sk_live_xxx            # API key
export DISCARD_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `discard_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `discard_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DISCARD_APIKEY` | API key sent with every request. |
| `DISCARD_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

ai_chat | test | testing

### Smoke test via HTTP (raw JSON-RPC)

```sh
./discard-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"discard_load","arguments":{"entity":"test","query":{"id":1}}}}'
```

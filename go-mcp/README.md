...
MCP server exposing the Discard SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `discard_list` | `entity`, optional `query` map | First page of records as JSON |
| `discard_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `discard_list` and
`discard_load` tools become available in new sessions.
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"discard_load","arguments":{"entity":"ai_chat","query":{"id":1}}}}'
```

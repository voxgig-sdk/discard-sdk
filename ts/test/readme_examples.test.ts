...
//     (\`new DiscardSDK(...)\` / \`DiscardSDK.test(...)\` become
//     \`DiscardSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"ai_chat":{"test01":{"id":"test01"}},"test":{"test01":{"id":"test01"}},"testing":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DiscardSDK(...)\` and any
// \`DiscardSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DiscardSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DiscardSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

...
import (
	...
	omni "github.com/voxgig-sdk/discard-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DiscardError); is {
				out := map[string]any{
					"name":    "DiscardError",
...
func omniCtx(arg any, client *sdk.DiscardSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DiscardSDK); is {
			client = live

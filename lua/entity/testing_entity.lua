...
function TestingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TestingEntity)
  self._name = "testing"
  self._client = client
...
function TestingEntity:get_name()
  return self._name
end


function TestingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TestingEntity.new(self._client, opts)
end
...
function TestingEntity:mark_deleted()
  self._deleted = true
end


function TestingEntity:deleted()
  return true == self._deleted
end


function TestingEntity:data_set(args)
  if args ~= nil then
...
function TestingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TestingEntity:match_set(args)
  if args ~= nil then
...
function TestingEntity:match_get()
...
function TestingEntity:stream(action, args, callopts)
...
---@param reqmatch TestingLoadMatch
---@param ctrl? table
---@return Testing
---@return string? err
function TestingEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata TestingCreateData
---@param ctrl? table
---@return Testing
---@return string? err
function TestingEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function TestingEntity:_run_op(ctx, post_done)
  local utility = self._utility

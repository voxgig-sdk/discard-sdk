...
local AiChatEntity = {}
AiChatEntity.__index = AiChatEntity


function AiChatEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AiChatEntity)
  self._name = "ai_chat"
  self._client = client
...
function AiChatEntity:get_name()
  return self._name
end


function AiChatEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AiChatEntity.new(self._client, opts)
end
...
function AiChatEntity:mark_deleted()
  self._deleted = true
end


function AiChatEntity:deleted()
  return true == self._deleted
end


function AiChatEntity:data_set(args)
  if args ~= nil then
...
function AiChatEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AiChatEntity:match_set(args)
  if args ~= nil then
...
function AiChatEntity:match_get()
...
function AiChatEntity:stream(action, args, callopts)
...
function AiChatEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
The Lua SDK for the Discard API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:AiChat()` — each with the same small set of operations (`load`, `create`, `update`, `remove`, `patch`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
local testing, err = client:Testing():load()
```
...
```
lua/
├── discard_sdk.lua    -- Main SDK module
...
The main module (`discard_sdk`) exports the SDK constructor

...
The Lua SDK for the Discard API. Provides an entity-oriented interface using Lua conventions.
...
```lua
-- Create
local created, _ = client:AiChat(nil):create({ name = "Example" }, nil)
...
local result, err = client:Discard(nil):load(
  { id = "test01" }, nil
)

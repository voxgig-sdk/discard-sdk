...
DiscardSDK._make_feature = _make_feature


function DiscardSDK.new(options)
  local self = setmetatable({}, DiscardSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function DiscardSDK:options_map()
...
function DiscardSDK:get_utility()
  return Utility.copy(self._utility)
end


function DiscardSDK:get_root_ctx()
  return self._rootctx
end


function DiscardSDK:prepare(fetchargs)
...
function DiscardSDK:direct(fetchargs)
...
-- by passing it.
function DiscardSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function DiscardSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "DiscardSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function DiscardSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = DiscardSDK.new(sdkopts)
  sdk.mode = "test"
